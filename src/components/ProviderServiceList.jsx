...
export default function ProviderServiceList() {
  ...
  return (
    <div style={{marginLeft: "80px", marginTop:"30px"}}>
      ...
      <div className="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default w-full">
        ...
        <div className="px-5 py-8 flex justify-between">
          ...
          <div className="flex gap-5">
            ...
            <div>
            <button id="dropdown2" data-dropdown-toggle="dropdown" className="inline-flex items-center justify-center rounded-lg text-black bg-white box-border border border-black hover:bg-brand-strong focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 text-sm px-4 py-2.5 focus:outline-none" type="button">
              All
          ...
          <form className="max-w-md mx-auto ">
            ...
            <div className="relative rounded-xl">
              <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none rounded-xl">
              ...
              <input type="search" id="search" className="block w-full p-3 ps-9 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-xl focus:ring-brand focus:border-brand shadow-xs placeholder:text-body" placeholder="Search" required />
              <button type="button" className="absolute end-1.5 bottom-1.5 text-white bg-brand-primary hover:bg-brand-primary/90 focus:ring-4 focus:ring-brand-primary/50 font-medium rounded-lg text-xs px-3 py-1.5 focus:outline-none transition-colors">Search</button>
            </div>
          </form>
        ...
        <table className="w-full text-sm text-center rtl:text-right text-body font-medium ">
          <thead className="text-xs text-white bg-brand-dark border-b border-brand-primary/20">
            <tr>
          ...
          <tbody>
            {services.map((service) => (
              <tr key={service.id} className="bg-white border-b hover:bg-gray-50 transition-colors text-xs">
                <td className="px-3">{service.name}</td>
                <td className="px-3">{service.category}</td>
                <td className="px-3">{service.pricing}</td>
                <td className="px-3">
